...
public abstract class EntityTameableFlying extends EntityTameable implements ITameable, IFlyable
{
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Entity Attributes
    public static final    IAttribute          flyingSpeed   = new RangedAttribute(null,
                                                                                   Utils.getModString("flyingSpeed"),
                                                                                   1D, 0.0D,
                                                                                   Double.MAX_VALUE)
            .setDescription("Flying Speed").setShouldWatch(true);
    public static final    IAttribute          flyingYaw     = new RangedAttribute(null,
                                                                                   Utils.getModString("flyingYaw"), 25D,
                                                                                   0.0D,
                                                                                   Double.MAX_VALUE)
            .setDescription("Flying Yaw Speed").setShouldWatch(true);
    public static final    IAttribute          flyingPitch   = new RangedAttribute(null,
                                                                                   Utils.getModString("flyingPitch"),
                                                                                   20D, 0.0D,
                                                                                   Double.MAX_VALUE)
            .setDescription("Flying Pitch Speed").setShouldWatch(true);
    ///////////////////////////////////////////////////////////////////////////
    // Datawatcher
    ...
    /**
     * Detects if there are air blocks below the entities lowest bounding box position based on range, centered on
     * entity's x/z axis
     */
    public boolean isAirBelow(int range)
    {
        for (int curBlock = 1; curBlock <= range; curBlock++)
            if (!this.world.isAirBlock(new BlockPos(MathHelper.floor(this.posX),
                                                    MathHelper.floor(this.getEntityBoundingBox().minY) - curBlock,
    ...
    /**
     * Retrieves if this entity is inside a liquid material (provides a better cross-mod implementation then {@link
     * Entity#isInsideOfMaterial(Material)} )
     */
    ...
    public void moveFlying(float strafe, float forward, float flySpeed)
    {
        ...
        super.travel(strafe * flySpeed, (float) (forward * Math
                             .tan(Math.toRadians(MathHelper.clamp(-rotationPitch * 2, -45, 45))) * flySpeed + 0.133f),
                     forward * flySpeed * 2);
    }
    ...
    public void updateRidden()
    {
        ...
        {
            this.getRidingEntity().setPosition(this.posX + this.getMountedXOffset(),
                                               this.posY + this.getMountedYOffset() + this.getRidingEntity()
                                                                                          .getYOffset(),
                                               this.posZ + this.getMountedZOffset());
        }
    }
