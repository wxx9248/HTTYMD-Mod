...
public class EntityRegister
{
        ...
        register.add(() ->
                     {
                         if (nextID < 1)
                         {
                             ...
                             nextID += 1;
                         EntityRegistry.registerModEntity(new ResourceLocation(HTTYMDMod.ID, entityName), entityClass,
                                                          entityName, nextID, HTTYMDMod.INSTANCE, 50, 2, true);
                         addSpawnInfo(HTTYMDMod.ID + ":" + entityName, solidColor, spotColor);
                     });
        HTTYMDMod.registerDragonName(entityName);
		/*ItemSpawnEgg egg = (ItemSpawnEgg) new ItemSpawnEgg(entityName, solidColor, spotColor).setCreativeTab
		(currentTab);
		egg.registerItem();
    ...
    private static EntityList.EntityEggInfo addSpawnInfo(String id, int primaryColor, int secondaryColor)
    {
        ...
        EntityList.EntityEggInfo egg = new EntityList.EntityEggInfo(
                resourcelocation, primaryColor, secondaryColor);
