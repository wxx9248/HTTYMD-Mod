...
public class WorldItemRegistry
{
    ...
    public static void init()
    {
		...
		// Dungeon Chests
		hook = ChestGenHooks.getInfo(ChestGenHooks.DUNGEON_CHEST); // Doesn't make sense to waste resources with more
		then one stored instance
		...
		vreg.registerVillageTradeHandler(blacksmithId, new IVillageTradeHandler() {
			...
			@Override
			public void manipulateTradesForVillager(EntityVillager villager, MerchantRecipeList recipeList, Random
			random) {
				recipeList.add(new MerchantRecipe(new ItemStack(Items.EMERALD, random.nextInt(10)+5), ItemRegistry
				.crossbow));
				recipeList.add(new MerchantRecipe(new ItemStack(ItemRegistry.crossbow), new ItemStack(Items.emerald,
				random.nextInt(5)+1)));
			}
		});*/
    }
     ...
     */
	/*public static void addGeneratedItem(ChestGenHooks hook, Item i, int damage, int minSize, int maxSize, int
	weight) {
