...
public class ItemWarhammer extends ItemWeapon {
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot slot, ItemStack stack) {
		...
		if (slot == EntityEquipmentSlot.MAINHAND)
		{
			...
			multimap.put(SharedMonsterAttributes.ATTACK_SPEED.getName(), new AttributeModifier(ATTACK_SPEED_MODIFIER, "Weapon modifier", -3f, 2));
		}
	...
	public void onUpdate(ItemStack stack, World world, Entity user, int p_77663_4_, boolean inHand) {
		if(inHand && user instanceof EntityLivingBase) {
			...
			PotionEffect effect = new PotionEffect(Potion.REGISTRY.getObject(new ResourceLocation("mining_fatigue")), 1, 10, true, false);
