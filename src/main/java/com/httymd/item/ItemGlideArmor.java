...
public class ItemGlideArmor extends ItemArmorExtension implements ISpecialArmor
{
    ...
    public boolean canGlide(EntityLivingBase entity, ItemStack stack)
    {
        boolean canGlide = this.isFlyable(entity) && (this.isGliding(stack)
                                                      || (entity.motionY < -1.0 && entity.moveForward >= 0.1 && entity
                .isSneaking()));
    ...
    public void onArmorTick(World world, EntityLivingBase entity, ItemStack stack)
    {
		...
		/*Collection<AttributeModifier> c = entity.getAttributeMap().getAttributeInstance(SharedMonsterAttributes
		.ATTACK_DAMAGE).getModifiers();
        ...
        if (canGlide)
        {
            ...
            entity.motionX = -(Math.sin(
                    Math.toRadians(entity.getRotationYawHead())) * ((2 + entity.moveForward) / 3 * m));
            entity.motionZ = (Math.cos(
                    Math.toRadians(entity.getRotationYawHead())) * ((2 + entity.moveForward) / 3 * m));
            float tr = 0.2f;
    ...
    public String getArmorTexture(ItemStack stack, Entity entity, EntityEquipmentSlot slot, String type)
    {
		/*if (this.isGliding(stack) && slot == EntityEquipmentSlot.LEGS) {
			return HTTYMDMod.ID + ":textures/armor/"
					+ getRegistryName().getResourcePath().substring(0, getRegistryName().getResourcePath().lastIndexOf
					("_")) + "_fins.png";
		}*/
