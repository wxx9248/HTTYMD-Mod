...
import com.httymd.client.model.dragon.*;
...
public class ClientProxy extends CommonProxy
{
    ...
    private static void writeModel(String name, Item item)
    {
        ...
        else if (item instanceof ItemShieldM)
        {
            writeModel(name, name, "httymd:item/shield_base", w ->
            {
                ...
                w.println("  \"overrides\": [");
                w.println(
                        "    { \"predicate\": { \"blocking\": 1 }, \"model\": \"httymd:item/" + name + "_blocking\" }");
                w.print("  ]");
            });
    ...
    private static void addRenderToRegistry(Item item, int meta, String name)
    {
        ModelLoader.setCustomModelResourceLocation(item, meta,
                                                   new ModelResourceLocation(new ResourceLocation(HTTYMDMod.ID, name),
                                                                             "inventory"));
    }
    ...
    {
        // RenderingRegistry.registerEntityRenderingHandler(EntityGronkle.class, new RenderGronkle(new ModelGronkle()
        // , 1));
        RenderingRegistry.registerEntityRenderingHandler(EntityNightFury.class,
                                                         m -> new RenderDragon(m, new ModelNightFury(), 2));
        RenderingRegistry.registerEntityRenderingHandler(EntityNightmare.class,
                                                         m -> new RenderDragon(m, new ModelNightmare(), 2));
        RenderingRegistry.registerEntityRenderingHandler(EntityNadder.class,
                                                         m -> new RenderDragon(m, new ModelDeadlyNadder(), 2));
        RenderingRegistry
                .registerEntityRenderingHandler(EntitySkrill.class, m -> new RenderDragon(m, new ModelSkrill(), 2));
        RenderingRegistry.registerEntityRenderingHandler(EntityTerribleTerror.class,
                                                         m -> new RenderDragon(m, new ModelTerribleTerror(), 0.5F));
        // RenderingRegistry.registerEntityRenderingHandler(EntityZippleback.class, new RenderZippleback(new
        // ModelZippleback(), 1));
        RenderingRegistry.registerEntityRenderingHandler(EntityLightFury.class,
                                                         m -> new RenderDragon(m, new ModelNightFury(), 2));
    }
