...
public class PlayerClientHandler {
	...
	public void beforeBodyRender(RenderLivingEvent.Pre event) {
		...
		if (!(event.getEntity() instanceof EntityPlayer) || event.getRenderer() == this.glideRender || event.getEntity() != this.ent)
			return;
		...
		if (((ItemGlideArmor) stack.getItem()).isGliding(stack)) {
			glideRender.doRender((AbstractClientPlayer)event.getEntity(), event.getX(), event.getY() + event.getEntity().getEyeHeight(), event.getZ(), 0.0F, playerTicks);
